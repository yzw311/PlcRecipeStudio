...
public class ProtocolLoopbackTests
{
    ...
    public async Task ModbusTcp_字读写往返()
    {
        ...
        await client.WriteWordsAsync(new ParsedAddress("HR", 0, -1, false, "HR40001"), write.ToArray());
        ...
        var read = await client.ReadWordsAsync(new ParsedAddress("HR", 0, -1, false, "HR40001"), 3);
    ...
    public async Task ModbusTcp_位读写往返()
    {
        ...
        await client.WriteBitsAsync(new ParsedAddress("C", 0, -1, true, "C00001"), [true, false, true, true]);
        var bits = await client.ReadBitsAsync(new ParsedAddress("C", 0, -1, true, "C00001"), 4);
    ...
    public async Task ModbusTcp_异常响应_抛通讯异常()
    {
        ...
        await Assert.ThrowsAsync<PlcAddressException>(() =>
            client.WriteWordsAsync(new ParsedAddress("IR", 0, -1, false, "IR30001"), [1]));
        await client.DisposeAsync();
    }
    ...
    public async Task MelsecMc3E_字读写往返()
    {
        ...
        await client.WriteWordsAsync(new ParsedAddress("D", 100, -1, false, "D100"), w2.ToArray());
        Assert.Equal(1234, sim.DWords[100]);
        Assert.Equal(5678, sim.DWords[101]);

        var read = await client.ReadWordsAsync(new ParsedAddress("D", 100, -1, false, "D100"), 2);
    ...
    public async Task MelsecMc3E_位读写往返()
    {
        ...
        await client.WriteBitsAsync(new ParsedAddress("M", 10, -1, true, "M10"), [true, false, true]);
        var bits = await client.ReadBitsAsync(new ParsedAddress("M", 10, -1, true, "M10"), 3);
    ...
    public async Task MelsecMc3E_超过单帧上限_分块续传不重叠()
    {
        ...
        await client.WriteWordsAsync(new ParsedAddress("D", start, -1, false, $"D{start}"), words);
        ...
        var read = await client.ReadWordsAsync(new ParsedAddress("D", start, -1, false, $"D{start}"), (ushort)count);
        for (int i = 0; i < count; i++)
    ...
    public async Task OmronFins_字读写往返()
    {
        ...
        await client.WriteWordsAsync(new ParsedAddress("DM", 100, -1, false, "D100"), w3.ToArray());
        Assert.Equal(4321u, sim.DmWords[100]);
        Assert.Equal(8765u, sim.DmWords[101]);

        var read = await client.ReadWordsAsync(new ParsedAddress("DM", 100, -1, false, "D100"), 2);
    ...
    public async Task OmronFins_位读写往返()
    {
        ...
        await client.WriteBitsAsync(new ParsedAddress("DM", 50, 3, true, "D50.3"), [true]);
        var bits = await client.ReadBitsAsync(new ParsedAddress("DM", 50, 3, true, "D50.3"), 1);
