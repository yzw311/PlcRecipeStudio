...
public class ServiceIntegrationTests
{
    private static async Task<(TestHost Host, PlcDevice Device, Recipe Recipe)> SetupAsync(bool signal = false)
    {
        ...
        var device = await host.CreateMockDeviceAsync("1#机", signal).ConfigureAwait(false);
    ...
    public async Task 配方保存_版本递增_值落库()
    {
        ...
        await host.Recipes.SaveRecipeAsync(recipe.Id, rows, "admin").ConfigureAwait(false);
        var reloaded = await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false);
        ...
        await host.Recipes.SaveRecipeAsync(recipe.Id, rows, "admin").ConfigureAwait(false);
        reloaded = await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false);
    ...
    public async Task 配方重名_报错()
    {
        ...
        await using var _ = host;
        var device = await host.CreateMockDeviceAsync("1#机").ConfigureAwait(false);
        await host.Recipes.CreateRecipeAsync(device.Id, "标准配方", null, "admin").ConfigureAwait(false);
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            host.Recipes.CreateRecipeAsync(device.Id, "标准配方", null, "admin")).ConfigureAwait(false);
    }
    ...
    public async Task 非法变量值_保存被拒()
    {
        ...
        await using var _ = host;
        var device = await host.CreateMockDeviceAsync("1#机").ConfigureAwait(false);
        var recipe = await host.Recipes.CreateRecipeAsync(device.Id, "R", null, "admin").ConfigureAwait(false);
        var rows = TestHost.StandardRows();
    ...
    public async Task 下载_写入Mock并回读校验()
    {
        ...
        var result = await host.Transfers.DownloadAsync(device, recipe, null, verifyAfterWrite: true).ConfigureAwait(false);
    ...
    public async Task 上传_读到与配方一致的值()
    {
        ...
        // 先下载使 PLC 有值，再上传
        await host.Transfers.DownloadAsync(device, recipe).ConfigureAwait(false);
        var upload = await host.Transfers.UploadAsync(device, recipe).ConfigureAwait(false);
    ...
    public async Task 对比_一致时零差异_修改后报告差异()
    {
        ...
        await host.Transfers.DownloadAsync(device, recipe).ConfigureAwait(false);
        var cmp = await host.Compare.CompareAsync(device, recipe).ConfigureAwait(false);
        ...
        var cmp2 = await host.Compare.CompareAsync(device, recipe).ConfigureAwait(false);
    ...
    public async Task 写差异项_只写选中行()
    {
        ...
        var result = await host.Transfers.DownloadSubsetAsync(device, recipe, [speedRow]).ConfigureAwait(false);
        Assert.Equal(TransferStatus.Success, result.Status);
    ...
    public async Task 用户服务_哈希验证与角色()
    {
        ...
        Assert.Null(await host.Users.VerifyAsync("op1", "错误密码").ConfigureAwait(false));
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            host.Users.AddUserAsync("op1", "123456", UserRole.Operator, null)).ConfigureAwait(false);
    }
    ...
    public async Task 保存配方_UI路径_带Id行重复保存()
    {
        ...
        {
            var loaded = await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false);
            await host.Recipes.SaveRecipeAsync(recipe.Id, loaded.Items, "admin").ConfigureAwait(false);
        }
    ...
    public async Task 信号联动握手_下载请求_完成位闭环()
    {
        var host = await TestHost.CreateAsync().ConfigureAwait(false);
        await using var _ = host;
        var device = await host.CreateMockDeviceAsync("1#机", signal: true).ConfigureAwait(false);
        var recipe = await host.CreateRecipeWithRowsAsync(device, "标准配方").ConfigureAwait(false);
    ...
    public async Task 信号联动_上传未知配方名_自动创建配方()
    {
        ...
        {
            var recipes = await host.Recipes.GetRecipesAsync(device.Id).ConfigureAwait(false);
    ...
    public async Task 信号联动_配方名不存在_失败位置位()
    {
        ...
        await using var _ = host;
        var device = await host.CreateMockDeviceAsync("1#机", signal: true).ConfigureAwait(false);
        await host.CreateRecipeWithRowsAsync(device, "标准配方").ConfigureAwait(false);
        ...
        while (DateTime.UtcNow < deadline)
        {
            var (_, total) = await host.OpLogs.QueryAsync(null, null, null, TransferSource.SignalTrigger,
                "信号下载配方", false, 1, 10).ConfigureAwait(false);
    ...
    public async Task 设备编辑_配方名字段持久化()
    {
        ...
        await host.Devices.UpdateDeviceAsync(edited).ConfigureAwait(false);

        var reloaded = (await host.Devices.GetDevicesAsync().ConfigureAwait(false))
