...
public class HardeningP0Tests
{
    ...
    public async Task 保存配方_生成版本历史快照_含全部数据行()
    {
        ...
        var recipe = await host.Recipes.CreateRecipeAsync(device.Id, "标准配方", null, "admin").ConfigureAwait(false);

        var rows = TestHost.StandardRows();
        rows.ForEach(r => r.Recipe = recipe);
        await host.Recipes.SaveRecipeAsync(recipe.Id, rows, "admin", "首次保存").ConfigureAwait(false);

        var history = await host.Recipes.GetVersionHistoryAsync(recipe.Id).ConfigureAwait(false);
        ...
        var snapshot = await host.Recipes.GetVersionSnapshotAsync(recipe.Id, entry.Version).ConfigureAwait(false);
    ...
    public async Task 版本历史_按版本倒序_且多条保留()
    {
        ...
        await host.Recipes.SaveRecipeAsync(recipe.Id, rows, "admin", "修改模式").ConfigureAwait(false); // v3

        var history = await host.Recipes.GetVersionHistoryAsync(recipe.Id).ConfigureAwait(false);
        Assert.Equal(new[] { 3, 2, 1 }, history.Select(h => h.Version)); // 新建 v1 也留档
        var v2 = history.Single(h => h.Version == 2);
        var v2Rows = await host.Recipes.GetVersionSnapshotAsync(recipe.Id, v2.Version).ConfigureAwait(false);
    ...
    public async Task 回滚_把历史快照保存为新版本_值还原()
    {
        ...
        await using var _ = host;
        var device = await host.CreateMockDeviceAsync("1#机").ConfigureAwait(false);
        var recipe = await host.CreateRecipeWithRowsAsync(device, "标准配方").ConfigureAwait(false); // v2：温度=12.5

        // 模拟现场改值后保存（v3：温度=99）
        var rows = (await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false)).Items;
        rows.First(r => r.Name == "温度").Value = "99";
        await host.Recipes.SaveRecipeAsync(recipe.Id, rows, "admin", "现场改值").ConfigureAwait(false);

        // 回滚到 v2 快照
        var history = await host.Recipes.GetVersionHistoryAsync(recipe.Id).ConfigureAwait(false);
        var v2 = history.Single(h => h.Version == 2);
        var snapshot = await host.Recipes.GetVersionSnapshotAsync(recipe.Id, v2.Version).ConfigureAwait(false);
        await host.Recipes.SaveRecipeAsync(recipe.Id, snapshot, "admin", "回滚自 v2").ConfigureAwait(false);

        var final = await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false);
        Assert.Equal(4, final.Version);
        Assert.Equal("12.5", final.Items.First(i => i.Name == "温度").Value);
        Assert.Equal("回滚自 v2",
            (await host.Recipes.GetVersionHistoryAsync(recipe.Id).ConfigureAwait(false)).First(h => h.Version == 4).ChangeNote);
    }
    ...
    public async Task 保存配方_基于过期版本_被拒绝()
    {
        ...
        // 工作站 A 拿到 v2 的行，尚未保存
        var staleRows = (await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false)).Items;
        staleRows.First(r => r.Name == "温度").Value = "1";
        staleRows.ForEach(r => r.Recipe = recipe);

        // 工作站 B 先保存成功 → v3
        var freshRows = (await host.Recipes.GetRecipeAsync(recipe.Id).ConfigureAwait(false)).Items;
        freshRows.First(r => r.Name == "速度").Value = "200000";
        await host.Recipes.SaveRecipeAsync(recipe.Id, freshRows, "B").ConfigureAwait(false);

        // A 再保存（基于 v2）→ 拒绝
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            host.Recipes.SaveRecipeAsync(recipe.Id, staleRows, "A")).ConfigureAwait(false);
        Assert.Contains("已被其他用户修改", ex.Message);

        // 行不带 Recipe 导航（程序化调用，如信号路径）→ 跳过检查，兼容旧行为
        var legacyRows = TestHost.StandardRows();
        await host.Recipes.SaveRecipeAsync(recipe.Id, legacyRows, "signal").ConfigureAwait(false);
    }
    ...
    public async Task 登录_连续失败5次锁定_正确密码也被拒()
    {
        var host = await TestHost.CreateAsync().ConfigureAwait(false);
        await using var _ = host;
        await host.Users.AddUserAsync("op1", "123456", UserRole.Operator, null).ConfigureAwait(false);

        for (var i = 0; i < 5; i++)
            Assert.Null(await host.Users.VerifyAsync("op1", "wrong").ConfigureAwait(false));

        // 锁定中：正确密码也拒绝
        Assert.Null(await host.Users.VerifyAsync("op1", "123456").ConfigureAwait(false));
        ...
        var user = await db.Users.FirstAsync(u => u.UserName == "op1").ConfigureAwait(false);
        user.LockoutUntilUtc = DateTime.UtcNow.AddSeconds(-1);
        await db.SaveChangesAsync().ConfigureAwait(false);
        Assert.NotNull(await host.Users.VerifyAsync("op1", "123456").ConfigureAwait(false));
    }
    ...
    public async Task 登录成功_清零失败计数()
    {
        ...
        Assert.Null(await host.Users.VerifyAsync("op1", "wrong").ConfigureAwait(false));
        Assert.NotNull(await host.Users.VerifyAsync("op1", "123456").ConfigureAwait(false));
        Assert.Null(await host.Users.VerifyAsync("op1", "wrong").ConfigureAwait(false)); // 若计数未清零，这次会触发锁定
        Assert.NotNull(await host.Users.VerifyAsync("op1", "123456").ConfigureAwait(false));
    }
    ...
    public async Task 新建用户与内置管理员_强制首次改密_改密后解除()
    {
        var host = await TestHost.CreateAsync().ConfigureAwait(false);
        await using var _ = host;
        var admin = await host.Users.VerifyAsync(DbInitializer.DefaultAdminName, "Test-only-Admin-Password-123!").ConfigureAwait(false);
        ...
        await using var db = await host.Provider.GetRequiredService<IDbContextFactory<AppDbContext>>()
            .CreateDbContextAsync().ConfigureAwait(false);
        var op = await db.Users.FirstAsync(u => u.UserName == "op1").ConfigureAwait(false);
        Assert.True(op.MustChangePassword);

        await host.Users.ChangePasswordAsync(op.Id, "newpass1").ConfigureAwait(false);
        db.Entry(op).State = EntityState.Detached;
        op = await db.Users.FirstAsync(u => u.UserName == "op1").ConfigureAwait(false);
    ...
    public async Task 操作日志_新写入为UTC_查询边界按本地时间换算()
    {
        ...
        await host.OpLogs.AddAsync("测试动作", "目标", "详情", true).ConfigureAwait(false);
        await using var db = await host.Provider.GetRequiredService<IDbContextFactory<AppDbContext>>()
            .CreateDbContextAsync().ConfigureAwait(false);
        var log = await db.OpLogs.AsNoTracking().OrderByDescending(l => l.Id).FirstAsync().ConfigureAwait(false);
        ...
        var (_, total) = await host.OpLogs.QueryAsync(
            DateTime.Today, DateTime.Today.AddDays(1), null, null, null, null, 1, 50).ConfigureAwait(false);
        Assert.True(total >= 1);
    }
    ...
    public async Task 自动备份_生成快照并按保留份数清理()
    {
        ...
        try
        {
            ...
            await Task.Delay(1100).ConfigureAwait(false); // 文件名精确到毫秒，确保文件名不同
            await backup.BackupAsync().ConfigureAwait(false);
